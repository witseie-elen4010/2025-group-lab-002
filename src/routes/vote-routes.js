...
router.post('/', async (req, res) => {
  try {
    const { player_id, voted_for_id, round_number, game_id} = req.body;
    if (!player_id || !voted_for_id || !round_number || !game_id) {
      return res.status(400).json({ message: 'Missing required vote fields.' });
    }
    const vote = await Vote.create({ player_id, voted_for_id, round_number, game_id })
