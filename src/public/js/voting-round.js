...
class VotingRound {
    ...
    checkWinCondition() {
        ...
        const impostors = this.players.filter(p => p.role === "Undercover" || p.role === "Mr. White").length;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function initializeVoting() {
      ...
      votingRound.players.forEach((player) => {
        if (player.username !== votingRound.players[currentVoterIndex].username) {
    ...
    voteForm.addEventListener("submit", async (event) => {
      ...
      try {
        votingRound.castVote(votingRound.players[currentVoterIndex].username, voteFor);
